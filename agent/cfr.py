...
def calc_dis(a, b):
    if type(b) != torch.Tensor:
        try:
            ...
            assert False
    ret = torch.norm(a - b)
    ret = max(ret, torch.tensor(1e-3).to(ret.device))
    ret = ret ** (-2)
...
class CFRAgent:
    def __init__(self, state_dim, action_dim, n_latent_var, lr, betas, gamma, k_epochs, print_every=5000):
        self.lr = lr
        self.betas = betas
        self.gamma = gamma
        ...
        self.value_optimizer = torch.optim.Adam(self.policy_old.value_layer.parameters(), lr=lr)
        self.policy_optimizer = torch.optim.Adam(self.policy.action_layer.parameters(), lr=lr)
        self.pol_supervise_optimizer = torch.optim.Adam(self.policy_old.action_layer.parameters(), lr=lr)
        ...
        self.cfr_policy = torch.ones([action_dim]) / action_dim
    ...
    def value_supervise(self, value_state, value_pred):
        ...
        for t in range(self.k_epochs):
            ...
            if t % self.print_every == self.print_every - 1 or self.print_every == 1 or t == self.k_epochs - 1:
                print('episode: {}, v_loss: {}'.format(t, loss))
    ...
    def policy_supervise(self, obs, policys):
        ...
        for t in range(self.k_epochs):
            ...
            if t % self.print_every == self.print_every - 1 or self.print_every == 1  or t == self.k_epochs - 1:
                print('episode: {}, p_loss: {}'.format(t, loss))
            
            self.pol_supervise_optimizer.zero_grad()
            loss.backward()
            self.pol_supervise_optimizer.step()

    def policy_update(self, state, cur_values):
        # tmp = torch.Tensor(cur_values)
        # tmp = torch.max(tmp, torch.Tensor([0.]))
        # print('cur pol regret:')
        # print(tmp)

        # self.cfr_policy = torch.exp(torch.Tensor(cur_values))
        # self.cfr_policy = tmp / torch.sum(tmp) # torch.sum(self.cfr_policy)
        self.cfr_policy = torch.softmax(torch.Tensor(cur_values), -1)
...
class NPAAgent:
    def __init__(self, n_step, n_belief, beliefs, *args, **kwargs):
        ...
        for i in range(n_step):
            ...
            for j in range(n_belief):
                tmp_agents.append(CFRAgent(*args, **kwargs))
    ...
    def act(self, step, observation, start_num = -1, in_training=False):
        ...
        if start_num == -1:
            ...
            for i in range(self.n_belief):
                if in_training:
                    # action_prob = self.agents[step][i].policy.act(observation[1 + self.n_target:])
                    # action_prob = torch.stack([self.agents[step][i].cfr_policy])
                    action_prob = self.agents[step][i].cfr_policy
                else:
            ...
            if in_training:
                # action_probs = self.agents[step][start_num].policy.act(observation[1 + self.n_target:])
                action_probs = self.agents[step][start_num].cfr_policy
            else:
    ...
    def evaluate(self, step, observation, action, start_num = -1, in_training=False):
        if type(observation) != torch.Tensor:
            observation = torch.from_numpy(observation).float().to(device) 
        ...
        if start_num == -1:
            ...
            for i in range(self.n_belief):
                if in_training:
                    # action_prob = self.agents[step][i].policy.act(observation[:,self.n_target:])
                    action_prob = torch.stack([self.agents[step][i].cfr_policy])
                else:
                ...
                if first_enum:
                    ...
                    if in_training:
                        v_preds = 0
                        # v_preds = torch.add(v_preds, self.agents[step][i].policy.value_layer(observation[:,self.n_target:]) * w[i])
                    else:
                        v_preds = torch.add(v_preds, self.agents[step][i].policy_old.value_layer(observation[:,self.n_target:]) * w[i])
                else:
                    action_probs = action_prob * w[i]
                    if in_training:
                        # v_preds = self.agents[step][i].policy.value_layer(observation[self.n_target:]) * w[i]
                        v_preds = 0
                    else:
            ...
            if in_training:
                # action_probs = self.agents[step][start_num].policy.act(observation[:,self.n_target:])
                # v_pred = self.agents[step][start_num].policy.value_layer(observation[:,self.n_target:])
                v_pred = 0
                # print('in eval:')
                # print(self.agents[step][start_num].cfr_policy)
                action_probs = torch.stack([self.agents[step][start_num].cfr_policy])
            else:
    ...
    def update(self, step, state, start_num, vs):
        # print(opponent_strategy.shape)
        # print(state.shape)
        self.agents[step][start_num].policy_update(state[self.n_target + 1:], vs)
...
class AtkNPAAgent:
    ...
    def update(self, step, state, start_num, vs):
        ...
        for type_i in range(self.n_type):
            self.agents[type_i].update(step, state, start_num, vs[type_i])

    def act(self, step, observation, type_n=None, start_num = -1, in_training=False):
        if type_n != None:
