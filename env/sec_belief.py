...
class SecurityEnv(BaseEnv):
    def __init__(self, n_slots, n_types, prior, n_rounds, value_low=1., value_high=10., zero_sum=False, seed=None, record_def=False, export_gambit=False):
        ...
        # print('ob space')
        # print(n_types + n_types + self.ob_len)
    ...
    def _get_atk_ob(self, base_ob):
        return np.concatenate(([self.rounds_so_far], self.type_ob, self.belief, base_ob))
    ...
    def _get_ob(self):
        base_ob = self._get_base_ob(min(self.rounds_so_far, self.n_rounds - 1))
    ...
    def assess_strategies(self, strategies, verbose=False):
        ...
        def_br = self.defender_strategy_exploiter.run(defender_strategy, attacker_strategy)
    ...
    # def _get_atk_ob_1(self, target, belief, last_obs_atk) :
    ...
    def convert_to_atk_ob(self, history, t):
        if self.record_def:
            ob = np.zeros(shape=(max(1, self.n_rounds - 1), 2, self.n_slots + 1))
        else:
            ob = np.zeros(shape=(max(1, self.n_rounds - 1), self.n_slots + 1))
        r = len(history)
        # print(r)
        for i in range(r):
            if self.record_def:
    ...
    def convert_to_def_ob(self, history):
        ...
        for i in range(r, self.n_rounds - 1):
            if self.record_def:
                ...
                ob[i][self.n_slots] = 1.0
        ob = np.concatenate([[r], self.belief, [0.], ob[r - 1].reshape(-1)])
        return ob
            ...
            if encoded in self.cache:
                ...
                for def_ac in range(self.n_slots):
                    ...
                    for atk_ac in range(self.n_slots):
                        ...
                        cur_belief = deepcopy(self.env.belief)
                        self.env.update_belief(self.env.belief, atk_strategy_type[atk_ac])
    ...
    class _DefenderStrategyExploiter(object):
        ...
        def run(self, defender_strategy, attacker_strategy):
            self.strategy = defender_strategy
            self.atk_strategy = attacker_strategy
            ret = []
            for t in range(self.n_types):
                self._reset()
                self._recursive([], t)
