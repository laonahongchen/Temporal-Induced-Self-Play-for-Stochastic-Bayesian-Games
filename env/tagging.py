...
class TaggingEnv(BaseEnv):
    def __init__(self, prior, n_steps, is_atk=False):
        # self.zero_sum = True
        # self.payoff = [[1., -1.], [-1., 1.]]
        ...
        observation_spaces = [spaces.Box(low=0., high=8., shape=[8]), spaces.Box(low=0., high=8., shape=[8])]

        action_spaces = [spaces.Discrete(4), spaces.Discrete(6)]
    ...
    def _get_obs(self):
        ...
        atk_obs = np.concatenate((np.array([self.round_cnt]), self.belief, one_hot(self.n_targets, self.goal), atk_obs))
        if np.random.rand() < 0.8:
            def_obs = np.concatenate((np.array([self.round_cnt]), self.belief, one_hot(self.n_targets, self.goal), def_obs))
        else:
    ...
    def reset(self, debug=False, verbose=False):
        ...
        if self.is_atk:
            self.goal = self.type = np.random.choice(self.n_targets)
        else:
            self.goal = self.type = np.random.choice(self.n_targets, p=self.prior)
    ...
    def sub_reset(self, round, belief, debug=False, verbose=False):
        ...
        self.goal = self.type = np.random.choice(self.n_targets, p=belief)
